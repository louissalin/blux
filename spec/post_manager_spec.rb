...
describe PostManager do
	...
	context "when adding a title" do
		...
		it "should output an warning message if the title is not unique" do
			@manager.set_attribute('post.1', "title", 'title')

			STDERR.stub!(:<<)
			STDERR.should_receive(:<<).with("warning: title 'title' is not unique\n")
			@manager.set_attribute('post.2', "title", 'title')
		end

		it "should still change the value of the previous title" do
			STDERR.stub!(:<<)
			@manager.set_attribute('post.1', 'title', 'title')
			@manager.set_attribute('post.2', 'title', 'title2')
			@manager.set_attribute('post.2', 'title', 'title')

			@manager.get_attribute('post.2', "title").should == "title"
		end
	end
	...
	context "when adding a tag" do
		...
		it "should add a new tags attribute" do
			@manager.set_attribute('post.1', 'categories', "a tag")
			@manager.get_attribute('post.1', "categories").should == "a tag"
		end

		it "should add to an existing tag attribute if it already exists" do
			@manager.set_attribute('post.1', 'categories', "a tag")
			@manager.set_attribute('post.1', 'categories', "another tag")
			@manager.get_attribute('post.1', "categories").should == "a tag,another tag"
		end
	end
	...
	context "when deleting a tag" do
		before :each do
			File.open("#{@post_dir}/.post_index", 'w') do |f|
				f.write('{"post.1":{"categories":"tag1,tag2,tag3"}}')
			end
		...
		it "should remove the attribute from the post index" do
			@manager.delete_attribute('post.1', "categories") do
				'tag2'
			end
			
			@manager.get_attribute('post.1', "categories").should == "tag1,tag3"
		end
		...
		it "should not do anything if the tag does not exist" do
			...
			@manager.get_attribute('post.1', "categories").should == "tag1,tag2,tag3"
		end
		...
		it "should remove the attribute if there are no tags left" do
			...
			@manager.get_attribute('post.1', "categories").should == nil
		end
	end
