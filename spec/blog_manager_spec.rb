...
describe BlogManager do
	...
	context "when publishing" do
		...
		it "should send the proper command" do
			@manager.should_receive(:system).with("ruby blux.rb --convert -f draft5.67 | ruby wp_publish.rb -t no title --config #{@blux_rc} | ruby blux.rb --set_edit_url -f draft5.67")
		...
		it "should send the command with the title included if it exists" do
			...
			@manager.should_receive(:system).with("ruby blux.rb --convert -f draft5.67 | ruby wp_publish.rb -t bla --config #{@blux_rc} | ruby blux.rb --set_edit_url -f draft5.67")
			@manager.publish 'draft5.67'
		end
